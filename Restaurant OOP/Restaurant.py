class Restaurant:
    ...
    def pay_expense(self, amount, description):
        if amount < self.balance:
            self.expense += amount
            self.balance -= amount
            print(f'Paying for {description} which amount is: {amount}')
        else:
            print("Not enough money to pay {amount}")

    def pay_salary(self, employee):
        print(f'Paying salary for {employee.name} whose salary is: {employee.salary}')
        if employee.salary < self.balance:
            self.balance -= employee.salary
            self.expense += employee.salary
            employee.receive_salary()
        else:
            print(f'Today we are not able to pay you {employee.name}')

    def show_employees(self):
        print('-------- Printing the employees list -------')
